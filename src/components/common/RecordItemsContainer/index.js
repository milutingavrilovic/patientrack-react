...
function RecordItemsContainer(props) {
  ...
  const  sortMe = (e) =>{    
    ...
    if(bind === '0') {
      setFixList(Object.assign({}, {
        ...fixList,
        [type]: newItems
      }))
    } else {
      setRecordList(Object.assign({}, {
        ...recordList,
        [type]: newItems
      }))
    }
    ...
    return (
      <TableContainer>
        ...
        >
          <TableHead>
            <TableRow>
              <TableCell align="left" onClick={sortMe} data-bind={props.bind} data-type={props.type} data-col={'created_at'} data-sort={sortDate}>Date</TableCell>
              <TableCell align="center" onClick={sortMe} data-bind={props.bind} data-type={props.type} data-col={'asset'} data-sort={sortAsset}>Asset</TableCell>
              <TableCell align="right" onClick={sortMe} data-bind={props.bind} data-type={props.type} data-col={'name'} data-sort={sortName}>Lawyer</TableCell>
            </TableRow>
          </TableHead>
        </Table>
      </TableContainer>
  ...
  React.useEffect(() => {
    if(props.fixItemList && (props.fixItemList['todo'].length > 0 || props.fixItemList['complete'].length > 0)){
      ...
      const toDoItems = props.fixItemList['todo'].map(item => ({
        Id: item.id,
        Asset: item.asset,
        Name: item.company_lawyer.first_name + ' ' + item.company_lawyer.last_name,
    ...
    if(props.recordItemList && (props.recordItemList['todo'].length > 0 || props.recordItemList['complete'].length > 0)){
    ...
    return (
      <TableContainer>
        ...
        >
          ...
          />
          <TableBody>
              {stableSort(itemsExpand, getComparator(order, orderBy))
                .map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="center" className={classesMat.tableHeader}>{row.Name}</TableCell>
                  ...
                  <Table className={`head_box_table `} size="small" aria-label="a dense table">
                    <TableBody>                    
                      ...
                      <TableRow key={2}>
                        ...
                        <TableCell>                          
                          <Typography variant="h6" component="h6" className={"green"} align="right">
...
const mapStateToProps = state => {
  ...
  return {
    ...
    fixItemCount: fixItems && fixItems.count ? fixItems.count[0].count_items : 0,
    fixItemList: (fixItems && fixItems.list) ? fixItems.list : { todo: [], complete: [] },
